...
test('Deberia poder realizar una multiplicación entre dos numeros negativos', async ({ page }) => {
  ...
  const num1 = getRandomNegativeNumber();
  const num2 = getRandomNegativeNumber();

  await page.getByRole('button', { name: '-' }).click()
  await page.getByRole('button', { name: Math.abs(num1).toString() }).click()
  await page.getByRole('button', { name: '*' }).click()
  await page.getByRole('button', { name: '-' }).click()
  await page.getByRole('button', { name: Math.abs(num2).toString() }).click()
